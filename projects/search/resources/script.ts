...
const searchmodule = (function () {
  const $ = document;
  const itemsContainer : HTMLElement = $.getElementById("itemsContainer");
  const buttons : HTMLCollection = $.getElementsByTagName("button");
  const itemsCounter : HTMLElement= $.querySelector(".itemCounterContainer h3");
  const searhContainer : HTMLElement = $.getElementById("search");
  ...
  const createListItems = (data : string) : void  => {
    fragmentHTML = "";
    data.map((item, index) => {
